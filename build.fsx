...
let product = "TrackableData"
let authors = [ "Esun Kim" ]
let copyright = "Copyright © 2015 Saladbowl Creative"
let company = "Saladbowl Creative"
let description = " TrackableData types can track data change and use it"
let tags = ["trackable";"data";"orm"]
...
Target "Build" <| fun _ ->
    ...
    !!"./TrackableData.sln"
    |> MSBuild "" "Rebuild" [("Configuration","Release-Net35")]
    |> ignore
...
module Nuget = 
    // add project dependency for other projects
    let getProjectDependency project =
        match project with
        | "TrackableData" -> [ ]
...
let createNugetPackages _ =
    ...
    for nuspec in !! "./core/**/*.nuspec" do
        ...
        if nuspec.Contains "Templates" then do
            ...
            let dependencies = getProjectDependency project
            NuGetHelper.NuGet
                (fun p ->
                    { p with
            ...
            let release35Dir = outputDir @@ project @@ @"bin\Release-Net35"
            let packages = projectDir @@ "packages.config"
            let packageDependencies = if (fileExists packages) then (getDependencies packages) else []
            let dependencies = packageDependencies @ getProjectDependency project
            let releaseVersion = getProjectVersion project
            ...
            if Directory.Exists(release35Dir) then do
                ...
                !! (release35Dir @@ project + ".dll")
                ++ (release35Dir @@ project + ".pdb")
                ++ (release35Dir @@ project + ".xml")
                ++ (release35Dir @@ project + ".ExternalAnnotations.xml")
