    ...
    public class TrackableDictionarySqlMapper<TKey, TValue> where TValue : new()
    {
                                            ...
                                            string keyColumnName, string valueColumnName = null)
        {
            ...
            _allColumnString = _headKeyColumn != null
                                   ? _headKeyColumn.Value.Name + "," + _allColumnStringExceptHead
                                   : _allColumnStringExceptHead;
        }

        /*
        public async Task<TrackableDictionary<TKey, TValue>> LoadAsync(DbSession session, object headKey)
        {
            // SELECT 쿼리문 구축
        ...
        public async Task SaveAsync(DbSession session, object headKey, TrackableDictionaryTracker<TKey, TValue> tracker)
        {
        ...
        */
        ...
        public string GenerateSaveSql(object headKey, TrackableDictionaryTracker<TKey, TValue> tracker)
        {
            ...
            foreach (var i in tracker.ChangeMap)
            {
                ...
                switch (i.Value.Operation)
                {
                    case TrackableDictionaryOperation.Add:
                        ...
                        if (_headKeyColumn != null)
                        {
                            sqlAdd.Append(_headKeyColumn.Value.ConvertToSqlValue(headKey));
                            sqlAdd.Append(",");
                    ...
                    case TrackableDictionaryOperation.Modify:
                        ...
                        if (_headKeyColumn != null)
                        {
                            sqlModify.Append(_headKeyColumn.Value.Name).Append("=");
                            sqlModify.Append(_headKeyColumn.Value.ConvertToSqlValue(headKey));
            ...
            if (removeIds.Any())
            {
                ...
                if (_headKeyColumn != null)
                {
                    sql.Append(_headKeyColumn.Value.Name).Append("=");
                    sql.Append(_headKeyColumn.Value.ConvertToSqlValue(headKey));
                    sql.Append(" AND ");
                }
