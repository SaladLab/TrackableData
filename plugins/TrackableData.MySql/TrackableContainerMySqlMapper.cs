...
{
    public class TrackableContainerMySqlMapper<T>
        where T : ITrackableContainer<T>
    {
        ...
        private class PropertyItem
        {
            ...
            public Func<MySqlConnection, object[], T, Task> LoadAndSetAsync;
            public Func<IContainerTracker<T>, object[], string> BuildSqlForSave;
        }

        private readonly PropertyItem[] PropertyItems;

        public TrackableContainerMySqlMapper(Tuple<string, object[]>[] mapperParameters)
        {
        ...
        private static PropertyItem[] ConstructPropertyItems(Tuple<string, object[]>[] mapperParameters)
        {
            ...
            foreach (var property in typeof(T).GetProperties())
            {
                ...
                {
                    typeof(TrackableContainerMySqlMapper<T>)
              ...
              where TPoco : ITrackablePoco<TPoco>
        {
            ...
            var mapper = new TrackablePocoMySqlMapper<TPoco>((string)mapperParameters[0],
        ...
        private static void BuildTrackableDictionaryProperty<TKey, TValue>(PropertyItem item, object[] mapperParameters)
        {
            ...
            var mapper = mapperParameters.Length == 3
                             ? new TrackableDictionaryMySqlMapper<TKey, TValue>(
        ...
        public async Task<int> CreateAsync(MySqlConnection connection, T value, params object[] keyValues)
        {
            var sql = new StringBuilder();
            foreach (var pi in PropertyItems)
            {
        ...
        public async Task<T> LoadAsync(MySqlConnection connection, params object[] keyValues)
        {
        ...
        public Task<int> SaveAsync(MySqlConnection connection, ITracker tracker, params object[] keyValues)
        {
            return SaveAsync(connection, (IContainerTracker<T>)tracker, keyValues);
        }

        public async Task<int> SaveAsync(MySqlConnection connection, IContainerTracker<T> tracker,
